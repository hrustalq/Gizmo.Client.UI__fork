...
document.addEventListener("DOMContentLoaded", () => {
  /** QR Code handler */
  async function enrichQRCode() {
    // Initialize parent element
    const parentEl = document.querySelector('.giz-alternative-login__qr-image');
    if (!parentEl) return;

    // Add loading state
    parentEl.classList.add('giz-alternative-login__qr-image--loading');
    try {
      // Get the SVG content
      const svgElement = parentEl.querySelector('svg');
      if (!svgElement) {
      ...
      const width = svgElement.width.baseVal.value || 170;
      const height = svgElement.height.baseVal.value || 170;
      canvas.width = width;
      ...
      const enrichedData = `x://activate?host=${HostId}&club=1`;
      
      // Generate new QR code
      const writer = new BrowserQRCodeSvgWriter();
      const newQrSvg = writer.write(enrichedData, width, height);
      
      // Add styling for thinner borders
      newQrSvg.style.transform = 'scale(1.2)';  // Scale down the QR code slightly
      
      // Replace old SVG with new one
      svgElement.parentNode.replaceChild(newQrSvg, svgElement);
      
      console.log('Successfully enriched QR code:', enrichedData);
    } catch (error) {
      console.error('Error processing QR code:', error);
    } finally {
      // Always remove loading state
      parentEl.classList.remove('giz-alternative-login__qr-image--loading');
    }
  } 

  const targetClass = '.giz-alternative-login__qr-image';
  let target = null;
